        ...
        @foreach ( $author->news as $new )
         <a href="{{ route('news.detail', $new->slug) }}">
          <div
            ...
            <img src="{{ asset('storage/' . $new->thumbnail) }}" alt="" class="w-full rounded-xl mb-3" style="height: 200px; object-fit: cover;">
            <p class="font-bold text-base mb-1">{{ $new->title }}</p>
            <p class="text-slate-400">{{ \Carbon\Carbon::parse($new->created_at)->format('d F Y') }}</p>
          </div>
        </a>
        @endforeach
